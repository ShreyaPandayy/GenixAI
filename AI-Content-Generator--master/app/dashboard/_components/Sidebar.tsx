...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, closeSidebar }) => {
  ...
  return (
    <>
      ...
      >
        {/* Sidebar Header */}
        <header className="flex flex-col items-start ml-5 py-4 border-b border-gray-200/30">
          ...
          {Menu.map((item) => {
            ...
            return (
              <Link
                href={item.link}
                key={item.text}
                className={`flex items-center gap-4 my-4 font-semibold text-lg rounded-xl transition-all duration-300 relative px-5 py-3 group ${
                  isActive
